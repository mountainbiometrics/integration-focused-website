...
export function generateStaticParams() {
  ...
  // Return a placeholder that notFound() will handle at render time.
...
function SectionBlock({ section, sectionIndex }: { section: MicrositeSection; sectionIndex: number }) {
  if (section.type === 'bullets') {
    ...
    const bulletSectionBg = isPain ? 'bg-[rgba(172,31,45,0.04)]' : 'bg-white';
    return (
              ...
              <div key={item.label} className="flex gap-3 md:gap-4">
                {isPain ? (
                  <span className="flex-shrink-0 w-5 h-5 mt-0.5 rounded-full bg-[rgba(172,31,45,0.10)] flex items-center justify-center">
                    <X className="w-3 h-3 text-[var(--ms-accent)]" strokeWidth={2.5} />
                  </span>
                ) : (
  ...
  if (section.type === 'stats') {
    ...
    const sectionBg = isPain
      ? 'bg-[rgba(172,31,45,0.06)]'
      : isSolution
        ? sectionIndex % 2 === 0 ? 'bg-white' : 'bg-[rgba(74,111,165,0.05)]'
        : sectionIndex % 2 === 0 ? 'bg-white' : 'bg-[var(--ms-hero-gradient)]';
    const accentBorder = isPain
      ? 'border-t-[3px] border-t-[var(--ms-accent)]'
      : isSolution
        ? 'border-t-[3px] border-t-[var(--ms-primary)]'
        : 'border-t-[3px] border-t-[var(--ms-primary)]';
        ...
        <div className="container-site max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {section.items.map((item) => (
              <div
                key={item.label}
                className="flex items-start gap-4 p-4 rounded-lg border border-[var(--ms-border)]"
              >
                <div className="flex-1">
                  <p className="font-medium text-[var(--ms-heading)]">
                    {item.label}
                  </p>
                  {item.detail && (
...
export default async function MicrositePage({
  ...
  return (
    <MicrositeThemeWrapper theme={config.theme}>
      ...
      <header className="py-5 bg-white border-b border-[var(--ms-border)]">
      ...
      >
        ...
        <div className="container-site max-w-3xl mx-auto text-center relative">
          <h1 className="text-3xl md:text-5xl font-bold text-[var(--ms-heading)] mb-6 md:mb-8">
            {config.headline}
          </h1>

          {config.heroStat && (
            <div className="inline-flex flex-col items-center bg-white/70 backdrop-blur-sm rounded-xl px-6 py-5 sm:px-10 sm:py-6 shadow-sm border border-[var(--ms-border)] border-t-[3px] border-t-[var(--ms-primary)] mb-6">
              <div className="w-10 h-10 rounded-full bg-[rgba(172,31,45,0.08)] flex items-center justify-center mb-3">
                <Activity className="w-5 h-5 text-[var(--ms-primary)]" strokeWidth={1.5} aria-hidden="true" />
              </div>
              <p className="text-2xl md:text-4xl font-bold text-[var(--ms-primary)]">
                {config.heroStat}
              </p>
          ...
          <a
            href="#download"
            className="inline-flex items-center justify-center gap-2 mt-6 md:mt-8 px-8 py-4 bg-[var(--ms-primary)] text-white font-medium rounded-md hover:opacity-90 transition-opacity"
          >
            ...
            <div className="flex justify-center py-3 md:py-5">
              <div className="flex items-center gap-3 w-full max-w-3xl mx-auto px-4">
                ...
                <div className="w-8 h-8 rounded-full bg-[rgba(74,111,165,0.08)] flex items-center justify-center flex-shrink-0">
                  <ChevronDown className="w-4 h-4 text-[var(--ms-primary)]" strokeWidth={2} aria-hidden="true" />
                </div>
                <div className="flex-1 h-px bg-[var(--ms-border)]" />
              </div>
            </div>
